...
#define COPY_EXP_NODE(from, to)              \
    ...
    to->v = from->v;                         \
    to->t = from->t;
#define COPY_tv_NODE(from, to) \
    to->v = from->v;           \
    to->t = from->t;
#define COPY_tv_nt_NODE(from, to) \
    to->v = from.name;            \
    to->t = from.type;
#define COPY_nt_tv_NODE(from, to) \
    to.name = from->v;            \
    to.type = from->t;
...
void frontend::Analyzer::com_FuncDef(AstNode *root)
{
    ...
    if (root->children.size() == 6)
    {
        ...
        for (int i = 0; i < result.size(); i++)
        {
            ...
            if (!flag_quan)
            {
                s.operand.name += std::to_string(scnum);
                scnum++;
            }
    ...
    if ((dynamic_cast<Term *>(root->children[1]))->token.value == "[")
    {
        ...
        if (flag_quan)
        {
            if (type == ir::Type::Int)
                ...
                program->globalVal.emplace_back(ir::Operand((dynamic_cast<Term *>(root->children[0]))->token.value, ir::Type::FloatPtr), num);
        }
        // 插入符号表
    ...
    else
    {
        ...
        if (flag_quan)
        {
            if (type == Type::Int)
                ...
                program->globalVal.emplace_back(ir::Operand((dynamic_cast<Term *>(root->children[0]))->token.value, ir::Type::Float));
        }
        // 插入符号表
...
int frontend::Analyzer::com_Stmt(Stmt *root)
{
    ...
    if (dynamic_cast<Term *>(root->children[0])->token.value == "return")
    {
        ...
        if (root->children.size() > 2)
        {
            ...
            ir::Operand s = ir::Operand("t" + std::to_string(scnum), ir::Type::Int);
            scnum++;
            ir::Operand s1 = ir::Operand("t" + std::to_string(scnum), ir::Type::Float);
            scnum++;
            ir::Operand s2 = ir::Operand("t" + std::to_string(scnum), ir::Type::Float);
            scnum++;
            ir::Operand s3 = ir::Operand("t" + std::to_string(scnum), ir::Type::Int);
            scnum++;
    ...
    if (root->children.size() > 1)
    {
        ...
        for (int i = 0, j = 0; i < muls.size() - 1, j < nums.size() - 1; i++, j++)
        {
            ...
            else
            {
                ir::Operand re1("t" + std::to_string(scnum), ir::Type::Int);
                scnum++;
                ir::Operand re2("t" + std::to_string(scnum), ir::Type::Int);
                scnum++;
                ir::Operand op2 = Operand(std::to_string(muls[i]), ir::Type::IntLiteral);
                nowfunc->addInst(new ir::Instruction(op2, ir::Operand(), re1, ir::Operator::def));
        ...
        for (int i = 0; i < (root->children.size() - 1) / 2; i++)
        {
            ...
            if (flag)
            {
                ...
                if (dynamic_cast<Term *>(root->children[2 * i + 1])->token.value == "*")
                {
                    ...
                    nowfunc->addInst(new Instruction(des2, w1, des2, ir::Operator::fdiv));
            }
            else
            {
                if (dynamic_cast<Term *>(root->children[2 * i + 1])->token.value == "*")
        ...
        for (int i = 0; i < (root->children.size() - 1) / 2; i++)
        {
            ...
            if (flag)
            {
                ...
                if (dynamic_cast<Term *>(root->children[2 * i + 1])->token.value == "+")
                {
                    ...
                    nowfunc->addInst(new Instruction(des2, w1, des2, ir::Operator::fsub));
            }
            else
            {
                if (dynamic_cast<Term *>(root->children[2 * i + 1])->token.value == "+")
...
void frontend::Analyzer::com_RelExp(RelExp *root)
{
    ...
    if (fir->t == ir::Type::Int)
        ...
        nowfunc->addInst(new ir::Instruction(ir::Operand(fir->v, fir->t), ir::Operand(), des1, ir::Operator::def));
    else if (fir->t == ir::Type::Float)
    {
        flag9 = 1;
